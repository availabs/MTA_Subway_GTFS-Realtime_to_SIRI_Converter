...
function getVehicleMonitoringServiceDelivery () { /*jshint validthis:true */
    ...
    var requestedTrains = this.GTFSrt.getAllMonitoredTrains();
            
    return {
        //"ResponseTimestamp"       : handled in the Cacher
        "VehicleMonitoringDelivery" : getVehicleMonitoringDelivery.call(this, requestedTrains) ,
 ...
 */
function getVehicleMonitoringDelivery (requestedTrains) { /*jshint validthis:true */
    return {
        //"ResponseTimestamp" : handled by the timestamper.
        "VehicleActivity"     : getVehicleActivity.call(this, requestedTrains) ,
 ...
 */
function getVehicleActivity (requestedTrains) { /*jshint validthis:true */
    var that = this;
        
    return requestedTrains.map(function (train_id) {
        return {
            "MonitoredVehicleJourney" : 
                getVehicleMonitoringMonitoredVehicleJourney.call(that, train_id) ,
            "RecordedAtTime" : 
                getRecordedAtTime.call(that, train_id) ,
        };
    });
}
 ...
 */
function getRecordedAtTime (train_id) {
    /*jshint validthis:true */
    var trip_id = this.GTFSrt.trainIDToTripIDMap[train_id] ;
...
function getValidUntil () {
    /*jshint validthis:true */
    var posixTimestamp = this.GTFSrt.getTimestamp() + _.get(this, ['config', 'gtfsrtConfig', 'readInterval'], NaN);

    return ( ! isNaN(posixTimestamp) ) ? timeUtils.getTimestampFromPosix(posixTimestamp) : null;
}
