...
function getMonitoredVehicleJourney (trip_id) {
    ...
    var train_id     = this.GTFSrt.tripIDToTrainIDMap[trip_id] ,
        ...
        gtfsTripKey  = this.GTFSrt.getGTFSTripKeyForRealtimeTripKey(trip_id) ,
    ...
    mvj = {
        ...
        "DestinationRef"           : getDestinationRef.call(this, gtfsTripKey, trip_id) ,
        "DestinationName"          : getDestinationName.call(this, gtfsTripKey, trip_id) ,
...
function getDirectionRef (trip_id) { //TODO: Is this right???
    /* jshint validthis: true */
    return this.GTFS.getDirectionIDForTrip(trip_id);
}
...
function getOriginRef (gtfsTripKey) {
    /* jshint validthis: true */
    var originId = this.GTFS.getOriginStopIdForTrip (gtfsTripKey),
 ...
 *   the vehicle is serving, prefixed by Agency ID."
 */
function getDestinationRef (gtfsTripKey, gtfsrtTripKey) {
    /* jshint validthis: true */
    var destinationId = this.GTFS.getDestinationStopIdForTrip(gtfsTripKey),
        mutator       = this.config.fieldMutators && this.config.fieldMutators.DestinationRef;

    if ((destinationId === null) && (this.config.gtfsrtConfig.use_last_stoptime_update_as_destination)) {
        destinationId = this.GTFSrt.getDestinationStopIdForTrip(gtfsrtTripKey);
    }
...
function getOriginAimedDepartureTime (trip_id) {
    /* jshint validthis: true */
    var departureTime = this.GTFSrt.getOriginTimeForTrip ?  this.GTFSrt.getOriginTimeForTrip(trip_id) : null ,
        startDate ,
        originId ,
        timeFormatString ;
    ...
    startDate = this.GTFSrt.getStartDateForTrip(trip_id) ;

    originId = this.GTFS.getOriginStopIdForTrip(trip_id) ;

    departureTime = this.GTFS.getScheduledDepartureTimeForStopForTrip(trip_id, originId) ;

    if (startDate) {
        timeFormatString = "YYYYMMDD_HH:mm:ss" ;
        departureTime = startDate + '_' + departureTime;
    } else {
        timeFormatString = "HH:mm:ss";
    }
    
    return (departureTime !== null) ? timeUtils.getTimestamp(departureTime, timeFormatString) : null;
}
