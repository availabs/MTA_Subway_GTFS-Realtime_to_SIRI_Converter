...
function getDestinationRef (trip_id) {
    /* jshint validthis: true */
    var destinationId = this.GTFS.getDestinationStopIdForTrip(trip_id),
...
function getOriginAimedDepartureTime (trip_id) {
    /* jshint validthis: true */
    var startDate     = this.GTFSrt.getStartDateForTrip(trip_id) ,
        departureTime = this.GTFSrt.getOriginTimeForTrip ?  this.GTFSrt.getOriginTimeForTrip(trip_id) : null ,
        originId ,
        timeFormatString ;
    ...
    // FIXME: Need to change to 'YYYYMMDD_HH:mm:ss'
