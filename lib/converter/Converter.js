 ...
 */
function Converter (GTFS, GTFSrt, config, trainTrackerInitialState) {
    ...
    try {
        ...
        this.trainTrackerSnapshot = new TrainTracker.newSnapshot(GTFS, GTFSrt, config, trainTrackerInitialState);
    } catch (e) {
        eventsCreator.emitError({ error: e, timestamp: (Date.now() + (process.hrtime()[1]%1000000)/1000000) });
    ...
    this.getState = function () {
        ...
        var state = new Buffer (
                        JSON.stringify({
                            gtfsrtTimestamp   : _this.GTFSrt.getTimestampForFeedMessage(),
                            GTFSrt_JSON       : _this.GTFSrt.GTFSrt_JSON ,
                            trainTrackerState : (_this.trainTrackerSnapshot && _this.trainTrackerSnapshot.getState()) ,
                        })
                    );
...
Converter.prototype.getTrainsWithAlertFilterObject = function () {
    return this.trainsWithAlertFilterObject;
};

Converter.prototype.getStopsWithAlertFilterObject = function () {
    return this.stopsWithAlertsFilterObject;
};

Converter.prototype.getRoutesWithAlertFilterObject = function () {
 ...
 */
Converter.prototype.getCompleteVehicleMonitoringResponse = function () {
    return {
        "Siri" : {
...
function getRecordedAtTime (trip_id) {
    ...
    var recordedAtTime = this.GTFSrt.getVehiclePositionTimestamp(trip_id) ;

    if (recordedAtTime) {
        return timeUtils.getTimestamp(recordedAtTime);
    } else {
        return null;
    }
}
...
function getValidUntil () {
    /*jshint validthis:true */
    var gtfsrtTimestamp     = this.GTFSrt.getTimestampForFeedMessage(),
