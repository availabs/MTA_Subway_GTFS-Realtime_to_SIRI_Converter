...
function newSnapshot (GTFS, GTFSrt, config) {
    ...
    console.info(++MESSAGE_COUNTER + ' : ' + gtfsrtTimestamp);
...
Snapshot.prototype = {
    ...
    getDistanceFromCall : function (gtfsTripKey, stop_id) {
        ...
        if (isNaN(trainDistAlongRoute_km) || isNaN(stopDistAlongRoute_km)) {
            ...
            distFromCall = ((stopDistAlongRoute_km - trainDistAlongRoute_km) * 1000).toPrecision(SIGNIFICANT_DIGITS);
...
function inferTrainLocation (GTFS, GTFSrt, trip_id) {
    ...
    try {
        ...
        try {
            var train_id = GTFSrt.getTrainIDForTrip(trip_id);

            console.error('ERROR: while tracking', gtfsTripKey, 'with train_id', train_id);
            console.error(e.message);

            if ( state.BAD_ETA ) {
                console.log('\tgtfsrtTimestamp       : ' + (+that.gtfsrtTimestamp) ? 
                                                                timeUtils.getTimestamp(+that.gtfsrtTimestamp) :
                                                                null);
                console.log('\tprevGtfsrtTimestamp   : ' + (previousSnapshot && previousSnapshot.gtfsrtTimestamp) ? 
                                                                timeUtils.getTimestamp(+previousSnapshot.gtfsrtTimestamp) :
                                                                null);
                console.log('\tpositionTimestamp     : ' + (+positionTimestamp) ? 
                                                                timeUtils.getTimestamp(+positionTimestamp) :
                                                                null);
                console.log('\tprevPositionTimestamp : ' + (previous && +previous.positionTimestamp) ? 
                                                                timeUtils.getTimestamp(+previous.positionTimestamp) : 
                                                                null);
                console.log('\timmediateNextStopETA  : ' + (immediateNextStop && +immediateNextStop.eta) ? 
                                                                timeUtils.getTimestamp(+immediateNextStop.eta) : 
                                                                null);
        ...
        if (state.NO_STOP_TIME_UPDATES) { immediateNextStop = 'DOES_NOT_EXIST'; }
        ...
        (function () {
            ...
            var debugging_info = {
                ...
                gtfsrtTimestamp       : (+that.gtfsrtTimestamp) ? 
                                            timeUtils.getTimestamp(+that.gtfsrtTimestamp) : 
                                            null ,
                prevGtfsrtTimestamp   : (previousSnapshot && +previousSnapshot.gtfsrtTimestamp) ? 
                                            timeUtils.getTimestamp(+previousSnapshot.gtfsrtTimestamp) : 
                                            null ,
                positionTimestamp     : (+positionTimestamp) ? 
                                            timeUtils.getTimestamp(+positionTimestamp) :
                                            null ,
                prevPositionTimestamp : (previous && +previous.positionTimestamp) ? 
                                            timeUtils.getTimestamp(+previous.positionTimestamp) : 
                                            null ,
                immediateNextStopETA  : (immediateNextStop && immediateNextStop.eta) ? 
                                            timeUtils.getTimestamp(+immediateNextStop.eta) : 
                                            null ,
