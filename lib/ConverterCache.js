...
ConverterCache.prototype.getStopMonitoringResponse = function (getParams, dataFormat, callback) {
    ...
    buildResponse.call(this, requestedTrains, 'stopMonitoring', 
                       stopMonitoringDetailLevel, maxOnwardCalls, stop_id, 
                       dataFormat, responseHandler.bind(null, 'stopMonitoring', callback, dataFormat));
};
...
ConverterCache.prototype.getVehicleMonitoringResponse = function (getParams, dataFormat, callback) {
    ...
    buildResponse.call(this, requestedTrains, 'vehicleMonitoring', 
                       vehicleMonitoringDetailLevel, maxOnwardCalls, null, 
                       dataFormat, responseHandler.bind(null, 'vehicleMonitoring', callback, dataFormat));
};


function responseHandler (callType, callback, dataFormat, err, response) {
    if (err) {
        callback(err);
    } else {
...
var generalResponseChunks = (function () {
    var shared_json = {
        ...
        shared_xml = {
            ...
            beginSituationExchangeDelivery : new Buffer('<SituationExchangeDelivery><Situations>'),

            endResponse                    : new Buffer('</Situations></SituationExchangeDelivery></ServiceDelivery></Siri>'),
        },
...
//console.log(generalResponsePiecesLength);
...
function buildResponse (requestedTrains, 
    ...
    var memoizationKey = requestedTrains.slice().sort().join('|') + '||' + 
                         Array.prototype.slice.call(arguments).slice(1,6).join('|');
    ...
    function getLengthOfResponse (callback) {
        try {
            ...
            //console.log('=1=> overallLen = ', overallLen);
            ...
            overallLen = requestedTrains.reduce(function (acc, train_id) {
                ...
                var metadata      = that.bufferedMonitoredVehicleJourneys.byTrainsIndex[dataFormat][train_id],
                    ...
                    monitoredCallLen = perTrainChunks[dataFormat].startMonitoredCall.length,
                    onwardCallsLen,
                ...
                if (that.bufferedCalls.byTrainsIndex[dataFormat][train_id]) { //Is the array of onwardcalls non-empty
                    ...
                    monitoredCallLen += callMetadata.length || 0;
                    ...
                    if ( onwardCalls.length && maxOnwardCalls && ((deliveryType === 'stopMonitoring') || (detailLevel === 'calls'))) {

                        // apply the maxOnwardCalls limit 
                        lastOnwardCallIndex = (maxOnwardCalls < onwardCalls.length) ?  (maxOnwardCalls - 1) : (onwardCalls.length - 1);
                        ...
                        if ((deliveryType === 'stopMonitoring') && (detailLevel !== 'calls')) {
                            ...
                            if (lastOnwardCallIndex < stopCallNumberForTrain) { lastOnwardCallIndex = stopCallNumberForTrain; }
                        } 
            ...
            //console.log('=2=> overallLen = ', overallLen);
            //console.log('=3=> requestedTrains = ', requestedTrains);
    ...
    function pipeTrain (train_id, respBuffer, offset, callback) {
        ...
        try {
            ...
            if (dataFormat === 'json') {
                offset -=2; //2 parens that end the MonitoredVehicleJourney //TODO If this is legit, need to -2 in getLengthOfResponse.
            }

            // Copy the beginning of the Monitored call
            offset += perTrainChunks[dataFormat].startMonitoredCall.copy(respBuffer, offset);
    ...
    function finito (err, respBuffer, offset) {
        ...
        if (err) {
            ...
            try {
                ...
                //console.log('=========================');
                //console.log("requestedTrains.length:", requestedTrains.length);
                //console.log("respBuffer.length:", respBuffer.length);
                //console.log("offset:", offset);
                //console.log("totalCalls:", totalCalls);
                //console.log('=========================\n');
                
                if (offset !== respBuffer.length) { console.log('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'); }
