    ...
    try {
        ...
        if ((allRoutes = this.GTFS.getAllRoutes())) {
            this.routesExistenceMap = allRoutes.reduce(function (acc, routeID) {
                if ((routeID !== null) && (routeID !== undefined) && (routeID.toString)) {
                    acc[routeID.toString().toLowerCase()] = 1;
                }
...
ConverterCache.prototype.getErrorResponse = function (message, callType, dataFormat, callback) {
    if (typeof message === 'string') {
        message = new QueryError(message);
    }

     responseBuilder.buildErrorResponse(message, callType, dataFormat,
            responseHandler.bind(null, callType, dataFormat, callback));
};
...
function operatorRefExists (GTFS, operatorRef) {
    ...
    for ( i=0; i < agency_ids.length; ++i ) {
        ...
        if (agency_id === 'MTA NYCT') { agency_ids.push('MTA'); }
