...
function bufferCalls (vehicleActivity) {
    ...
    for ( i = 0; i < vehicleActivity.length; ++i ) {
        ...
        datedVehicleJourneyRef = (mvj.FramedVehicleJourneyRef.DatedVehicleJourneyRef && 
                                  mvj.FramedVehicleJourneyRef.DatedVehicleJourneyRef.toLowerCase) ? 
                                        mvj.FramedVehicleJourneyRef.DatedVehicleJourneyRef.toLowerCase() : 
                                        mvj.FramedVehicleJourneyRef.DatedVehicleJourneyRef;

        // Note: We leave the unschedule trips' temporary DatedVehicleJourneyRefs so they can be
        //       used in bufferMonitoredVehicleJourneys.
...
function bufferMonitoredVehicleJourneys (vehicleActivity) {
    ...
    for ( i = 0; i < vehicleActivity.length; ++i ) {
        ...
        // Note: We do not convert the VehicleRef to lowercase yet because we may first need
        //       to escape the characters that will break xml responses.
        trip_id = mvj.VehicleRef;
        ...
        if ( xml_special_char_regex.test(trip_id) ) {
            mvj.VehicleRef = trip_id.replace(xml_special_char_regex, xml_char_escaper);
        } 
        ...
        trip_id = (mvj.VehicleRef && mvj.VehicleRef.toLowerCase) ?
                       mvj.VehicleRef.toLowerCase() : mvj.VehicleRef;

        this.vehicleRef_to_gtfsTripKey[trip_id] = gtfsTripKey; 
    }
