...
function bufferCalls (vehicleActivity) {
    ...
    for ( i = 0; i < vehicleActivity.length; ++i ) {
        ...
        for ( ii = 0; ii < onwardCalls.length; ++ii ) {
            ...
            /*========== Buffer the XML ===========*/           
            call_xml = '<OnwardCall>' + jsontoxml(call) + '</OnwardCall>';
...
function bufferMonitoredVehicleJourneys (vehicleActivity) {
    ...
    for ( i = 0; i < vehicleActivity.length; ++i ) {
        ...
        recordedAtTime = journey && journey.RecordedAtTime;
        ...
        /*========== Buffer the JSON ==========*/           
        journey_json = '{' +
                          '"RecordedAtTime":"'         + journey.RecordedAtTime + '",' +
                          '"MonitoredVehicleJourney":' + JSON.stringify(journey.MonitoredVehicleJourney) +
                       '}'
        ...
        // Because MonitoredCall and OnwardCalls nest inside MonitoredVehicleJourney,
        // we need to manually add the closing tag later.
        journey_xml = jsontoxml(journey).replace(/<\/MonitoredVehicleJourney>/, '');
