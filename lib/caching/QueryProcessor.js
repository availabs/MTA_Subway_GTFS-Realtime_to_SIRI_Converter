...
var QueryError = require('./QueryError.js') ;
...
function getRequestedTripKeysForVehicleMonitoringResponse (getParams) {
    ...
    if (operatorRef && (!_this.bufferedMonitoredVehicleJourneys.agency_ids[operatorRef])) { 
        if (operatorRefExists(_this.GTFS, operatorRef)) {
            return [];
        } else {
    ...
    gtfsTripKey = ((train_id !== undefined) && (train_id !== null)) ? 
                    (_this.vehicleRef_to_gtfsTripKey[train_id]) : null;

   
    /* If a tripKey is specified, return only that tripKey's trip trip. */
    if (gtfsTripKey) { 
        //Make sure tripKey exists in the byTripKeysIndex.
        if (_this.bufferedMonitoredVehicleJourneys.byTripKeysIndex.json[gtfsTripKey]) {
            return [gtfsTripKey];
        } 
    }

    if ( route_id && !isNaN(directionRef) ) { // Both route and directionRef are specified.
        reqTripKeysByRoute = [ _this.bufferedMonitoredVehicleJourneys //FIXME: Defensive code agains undefined.
                                       .partitionedByRouteByDirection[route_id][directionRef] || []];
    } else if ( route_id ) { // We have a route, but no direction.
        reqTripKeysByRoute = [ _this.bufferedMonitoredVehicleJourneys.partitionedByRoute[route_id] || []];
    } else if ( ! isNaN(directionRef) ) {  // Only direction specified.
        routes = Object.keys(_this.bufferedMonitoredVehicleJourneys.partitionedByRoute);

        reqTripKeysByRoute = routes.map(function (_route_id) { 
            return _this.bufferedMonitoredVehicleJourneys.partitionedByRouteByDirection[_route_id][directionRef]||[];
        });
        ...
        reqTripKeysByRoute = routes.map(function (_route_id) { 
             return _this.bufferedMonitoredVehicleJourneys.partitionedByRoute[_route_id] || [];
        });
    }

    if (route_id && Array.isArray(reqTripKeysByRoute) && reqTripKeysByRoute.length && !reqTripKeysByRoute[0].length){
        if (lineRefExists(_this.GTFS, route_id)) {
            return [];
        } else {
...
function getRequestedTripKeysForStopMonitoringResponse (stop_id, getParams) {
    ...
    var _this = this,
        ...
        operatorRef          = getParams.operatorref ,
    ...
    if (!this.GTFS.stopExists(stop_id)) {
        throw new QueryError('No such stop: ' + getParams._monitoringref + '.');
    }


    indexNodeForStop = _this.bufferedCalls.stopIDToCallNumberForTripKey[stop_id];
    ...
    //*********************************************
    //  Problem: MTA is hard-coded as operatorRef
    //
    //  Solution: Implement filtering on OperatorRef
    //            1) Make sure the specified OperatorRef (OR) is in the list of GTFS Agencies
    //                  a) If OR not in the list, throw error with message.



    if ( route_id && !isNaN(directionRef) ) { // Both route and directionRef are specified.
        reqTripKeyIndicesByRoute = [ _this.bufferedCalls
                                     .indicesOfTripKeysSortedByETAForStopByRoute[stop_id]
                                                                                [route_id]
                                                                                [directionRef] || []];

    } else if ( route_id ) { // We have a route, but no direction.
        reqTripKeyIndicesByRoute = [ _this.bufferedCalls.indicesOfTripKeysSortedByETAForStopByRoute[stop_id]
                                                                                              [route_id] || []];
    } else if ( ! isNaN(directionRef) ) {  // Only direction specified.
        routes = Object.keys(_this.bufferedCalls.indicesOfTripKeysSortedByETAForStopByRoute[stop_id]) || [];

        reqTripKeyIndicesByRoute = routes.map(function (_route_id) { 
             return _this.bufferedCalls.indicesOfTripKeysSortedByETAForStopByRouteByDirection[stop_id]
                                                                                          [_route_id]
                                                                                          [directionRef] || [];
        });
    } else { // Neither route nor direction specified.
        ...
        reqTripKeyIndicesByRoute = routes.map(function (_route_id) { 
             return _this.bufferedCalls.indicesOfTripKeysSortedByETAForStopByRoute[stop_id][_route_id] || [];
        });
    }


    if (!reqTripKeyIndicesByRoute.length) {
        if (lineRefExists(_this.GTFS, route_id)) {
            return [];
        } else {
            throw new QueryError('No such route: ' + getParams._lineref);
    ...
    return reqTripKeyIndices.map(function (tripKey_index) {
        return  _this.bufferedCalls.tripKeysSortedByETAForStop[stop_id][tripKey_index];
    });
}

function lineRefExists (GTFS, lineRef) {
    ...
    for ( i=0; i < agency_ids.length; ++i ) {
        ...
        if (GTFS.routeExists(strippedRouteID)) {
            return true;
        }
    }
