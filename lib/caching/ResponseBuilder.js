    ...
    try {
        ...
        overallLen = this.requestedTripKeys.reduce(function (acc, gtfsTripKey) {
            ...
            var metadata      = _this.bufferedMonitoredVehicleJourneys.byTripKeysIndex[dataFormat][gtfsTripKey],
                journeyLength = msgChunks.perJourney[deliveryType][dataFormat].startJourneyData.length + metadata[1],
            ...
            if (_this.bufferedCalls.byTripKeysIndex[dataFormat][gtfsTripKey] && 
                ...
                if (callMetadata.length) {
                    monitoredCallLen = msgChunks.perJourney[deliveryType][dataFormat].startNonemptyMonitoredCall.length + 
                                       callMetadata.length;

                    if (dataFormat === 'xml') {
                      monitoredCallLen -= ('<OnwardCall></OnwardCall>'.length)
                    }
                } else {
                    monitoredCallLen = msgChunks.perJourney[deliveryType][dataFormat].emptyMonitoredCall.length;
                }
                ...
                if ( onwardCalls.length && 
                        ...
                        ((deliveryType === 'stopMonitoring') || (_this.detailLevel === 'calls'))) {
                    ...
                    if ((deliveryType === 'stopMonitoring') && (_this.detailLevel !== 'calls')) {
                    ...
                    onwardCallsLen = msgChunks.perJourney[deliveryType][dataFormat].startNonemptyOnwardCalls.length +
                                     (((lastOnwardCall.offset + lastOnwardCall.length) - firstOnwardCall.offset)) +
                                     msgChunks.perJourney[deliveryType][dataFormat].endNonemptyOnwardCalls.length;

                    if (dataFormat === 'json') { --onwardCallsLen; }

                } else {
                    onwardCallsLen = msgChunks.perJourney[deliveryType][dataFormat].emptyOnwardCalls.length;
                }
             } else {
                monitoredCallLen = msgChunks.perJourney[deliveryType][dataFormat].emptyMonitoredCall.length;
                onwardCallsLen = msgChunks.perJourney[deliveryType][dataFormat].emptyOnwardCalls.length;
...
function pipeTrip (gtfsTripKey, respBuffer, offset, callback) {
    ...
    try {
        ...
        // Copy the beginning chunk of the trip's journey data.
        offset += msgChunks.perJourney[deliveryType][dataFormat].startJourneyData.copy(respBuffer, offset);
        ...
        if (this.bufferedCalls.byTripKeysIndex[dataFormat][gtfsTripKey] && 
            ...
            if (callMetadata.length) {
                ...
                offset += msgChunks.perJourney[deliveryType][dataFormat].startNonemptyMonitoredCall.copy(respBuffer, offset);

                offset += this.bufferedCalls.calls[dataFormat]
                                            .copy(respBuffer, offset, callOffset, callOffset);
                ...
                if (dataFormat === 'json') {
                  ...
                  offset += this.bufferedCalls.calls[dataFormat]
                                              .copy(respBuffer, offset, callOffset + '<OnwardCall>'.length, callOffset + callLen - '</OnwardCall>'.length);
                }

            } else {
                offset += msgChunks.perJourney[deliveryType][dataFormat].emptyMonitoredCall.copy(respBuffer, offset);
            }
            ...
            if ( onwardCalls.length  && 
                 this.maxOnwardCalls && 
                 ((deliveryType === 'stopMonitoring') || (this.detailLevel === 'calls'))
               ) {

                offset += msgChunks.perJourney[deliveryType][dataFormat].startNonemptyOnwardCalls.copy(respBuffer, offset);
                ...
                // In this case, we must include the calls the trip will make up to, 
                // and including, the selected stop.
                if ((deliveryType === 'stopMonitoring') && (this.detailLevel !== 'calls')) {
                    stopCallNumberForTrip = 
                ...
                offset += msgChunks.perJourney[deliveryType][dataFormat].endNonemptyOnwardCalls.copy(respBuffer, offset);

            } else {
                offset += msgChunks.perJourney[deliveryType][dataFormat].emptyOnwardCalls.copy(respBuffer, offset);
            }
