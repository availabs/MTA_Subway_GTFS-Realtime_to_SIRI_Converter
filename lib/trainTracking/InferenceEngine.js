...
var process = require('process') ,
    ...
    eventCreator = require('../events/EventCreators') ,
...
//var totalCt = 0,
    ...
    var result = allTrips.reduce(function (acc, trip_id) {
        ...
        try {
            ...
            if (!GTFS.tripIsAScheduledTrip(gtfsTripKey)) { 
                unscheduledTrips.push(gtfsTripKey) ; 
            } else if (!GTFS.tripsHasSpatialData(gtfsTripKey) ) { 
                noSpatialDataTrips.push(gtfsTripKey) ; 
            } else { 

                acc[gtfsTripKey] = locationInferer(trip_id) ; 
//++totalCt ;
//if (acc[gtfsTripKey].locationGeoJSON) { ++withLoc; }
            }
        } catch (err) {
...
//console.log('++>', withLoc + '/' + totalCt) ;

    // These are appended to the result for use by analysis tools.
    result.__unscheduledTrips   = unscheduledTrips ;
    result.__noSpatialDataTrips = noSpatialDataTrips ;
...
//}
    ...
    try {
        ...
        context.errorTimestamp    = (Date.now() + (process.hrtime()[1]%1000000)/1000000) ;
...
function getStateOfTrain (GTFS, GTFSrt, gtfsTripKey, trip_id, immediateStopInfo, previous) {
    ...
    if (previous && (immediateStopInfo.timestamp > previous.immediateStopInfo.timestamp)) {
        ...
        // If not dwelling at a stop, then the train moved.
        } else if ( !(previous.state.AT_STOP && state.AT_STOP && 
                      (previous.immediateStopInfo.stopId === immediateStopInfo.stopId)) ) {
