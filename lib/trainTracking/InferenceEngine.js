...
//          because estimatedTime is not being updated even though the train is moving.
//          Sometimes (rarely) an estimatedTime will be negative, even though the train 
...
function inferTrainLocation (GTFS, GTFSrt, trip_id) {
    ...
    var gtfsTripKey       = GTFSrt.getGTFSTripKeyForRealtimeTripID(trip_id) ,
        ...
        immediateStopInfo = trainTrackerUtils.getInfoForImmediateStop(GTFSrt, trip_id),
    ...
    try {
        ...
        if ( state.AT_STOP && 
            ...
            //TODO: Get the subsequent stop_id from the GTFS data.
            locationGeoJSON = trainTrackerUtils.getLineStringBetweenStopsForTrip(_GTFS,
                                                                                 gtfsTripKey, 
                                                                                 immediateStopInfo.stop_id, 
            ...
            if ( !state.HAS_MOVED ) { 
                ...
                //ratioCovered = (this.gtfsrtTimestamp  - previousSnapshot.gtfsrtTimestamp) / 
                               //(immediateStop.estimatedTime - previousSnapshot.gtfsrtTimestamp);

                ratioCovered = (positionTimestamp  - previous.positionTimestamp) / 
                               (immediateStopInfo.estimatedTime - previous.positionTimestamp);
...
//      missing instances of trains at stops. The other method uses the estimatedTime of the immediate next stop.
//      If the estimatedTime is less than the GTFS-Realtime timestamp, we could assume the train is at the stop.
...
function getStateOfTrain (GTFS, GTFSrt, trip_id, immediateStopInfo, previous) {
    ...
    //immediateStop = immediateStopInfo[0];
   
    state.AT_ORIGIN = ( !state.WAS_ASSIGNED_TRAIN) || 
                        (immediateStopInfo.estimatedTime === null) || 
                        (immediateStopInfo.estimatedTime === undefined );
