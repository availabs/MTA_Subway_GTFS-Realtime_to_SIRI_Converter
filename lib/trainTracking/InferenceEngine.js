...
function inferTrainLocation (GTFS, GTFSrt, trip_id) {
    ...
    var gtfsTripKey       = GTFSrt.getGTFSTripKeyForRealtimeTripID(trip_id) ,
        ...
        immediateStopEstimatedTime = trainTrackerUtils.getExpectedTimeForFirstStopInStopTimeUpdates(GTFSrt, trip_id),
    ...
    try {
        ...
        // TODO: Put this in an extract function.
        immediateStop  = stopTimeUpdates[0];
        ...
        if ( state.AT_STOP && 
            ...
            if ( !state.HAS_MOVED ) { 
                ...
                //ratioCovered = (this.gtfsrtTimestamp  - previousSnapshot.gtfsrtTimestamp) / 
                               //(immediateStop.eta - previousSnapshot.gtfsrtTimestamp);

                ratioCovered = (positionTimestamp  - previous.positionTimestamp) / 
                               (immediateStop.eta - previous.positionTimestamp);
        ...
        context.state             =  state ;           
        context._GTFS             =  _GTFS ;
        context.immediateStop     =  immediateStop ;
...
function getStateOfTrain (GTFSrt, trip_id, stopTimeUpdates, previous) {
    ...
    state.AT_ORIGIN = ( !state.WAS_ASSIGNED_TRAIN) || 
                        (stopTimeUpdates[0].eta === null) || 
                        (stopTimeUpdates[0].eta === undefined );
    ...
    state.SAME_IMMEDIATE_NEXT_STOP = ( !!(previous && previous.immediateStop) ) && 
                                     (immediateStop.stop_id === previous.immediateStop.stop_id) ;
