...
var turf = require('turf') ,
    ...
    timeUtils = require('MTA_Subway_GTFS_Realtime_Toolkit').TimeUtils;
...
function getLineStringBetweenStopsForTrip (_GTFS, gtfsTripKey, immediateStopID, subsequentStopID) {
    ...
    // Create the linestring from immediateStop to subsequentStop.
...
function getExpectedTimeForFirstStopInStopTimeUpdates (GTFS, GTFSrt, trip_id) {
    var stopTimeUpdates = GTFSrt.getStopTimeUpdatesForTrip(trip_id),
        update,
    ...
    if ( ! (Array.isArray(stopTimeUpdates) && stopTimeUpdates.length) )  { 
        return null; 
    }

    update = stopTimeUpdates[0];

    if ( (update === null) || (typeof object !== 'object')) { return null; }
    ...
    // Try to get a directly stated expected arrival time:
    expectedArrivalTimestamp = (update.arrival && update.arrival.time);
    expectedArrivalTimestamp = (expectedArrivalTimestamp !== null) && 
                               ( (!isNaN(expectedArrivalTimestamp) && expectedArrivalTimestamp) || 
                                 expectedArrivalTimestamp.low || expectedArrivalTimestamp.high   );

    if (expectedArrivalTimestamp) { return expectedArrivalTimestamp; }


    // Try to get expected arrival time using stated arrival delay.
    arrivalDelay = (update.arrival && update.arrival.delay);

    if ((arrivalDelay !== null) && (arrivalDelay !== undefined)) { 

        scheduledArrivalTime = GTFS.getScheduledArrivalTimeForStopForTrip(trip_id, stop_id, stop_sequence);

        if (scheduledArrivalTime !== null) {
            // 'X' is the output format for Unix Timestamps.
            scheduledArrivalTimestamp = timeUtils.getTimestamp(scheduledArrivalTime, "HH:mm:ss", 'X');

            if ((scheduledArrivalTimestamp !== null) && 
                    (scheduledArrivalTime = parseInt(scheduledArrivalTime)) ) {

                arrivalDelay = parseInt(arrivalDelay);
                return (!isNaN(arrivalDelay)) ? scheduledArrivalTimestamp + arrivalDelay : null;
            }
        }
    }

    // Try to get a directly stated departure time:
    expectedDepartureTimestamp = (update.departure && update.departure.time);
    expectedDepartureTimestamp = (expectedDepartureTimestamp !== null) && 
                               ( (!isNaN(expectedDepartureTimestamp) && expectedDepartureTimestamp) || 
                                 expectedDepartureTimestamp.low || expectedDepartureTimestamp.high   );

    if (expectedDepartureTimestamp) { return expectedDepartureTimestamp; }


    // Try to get expected departure time using stated departure delay.
    departureDelay = (update.departure && update.departure.delay);

    if ((departureDelay !== null) && (departureDelay !== undefined)) { 

        scheduledDepartureTime = GTFS.getScheduledDepartureTimeForStopForTrip(trip_id, stop_id, stop_sequence);

        if (scheduledDepartureTime !== null) {
            // 'X' is the output format for Unix Timestamps.
            scheduledDepartureTimestamp = timeUtils.getTimestamp(scheduledDepartureTime, "HH:mm:ss", 'X');

            if ( (scheduledDepartureTimestamp !== null) && 
                    (scheduledDepartureTime = parseInt(scheduledDepartureTime)) ) {

                departureDelay = parseInt(departureDelay);
                return (!isNaN(departureDelay)) ? scheduledDepartureTimestamp + departureDelay : null;
            }
        }
    }
...
module.exports = {
    ...
    getExpectedTimeForFirstStopInStopTimeUpdates : getExpectedTimeForFirstStopInStopTimeUpdates ,
