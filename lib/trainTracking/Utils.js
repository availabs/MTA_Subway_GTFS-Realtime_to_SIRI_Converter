...
function getImmediateStopInfo (GTFS, GTFSrt, trip_id) {
   ...
   // Get the most specific timestamp available.
    ...
    if (stopInfo.eta === null) { return stopInfo; }

    // The CurrentStatus data seems unreliable.
    //stopInfo.atStop = (GTFSrt.getVehiclePositionCurrentStatusForTrip &&
                       //GTFSrt.getVehiclePositionCurrentStatusForTrip(trip_id) ) ;

    //if (stopInfo.atStop) {
        //stopInfo.atStop = (stopInfo.atStop === 'STOPPED_AT');
    //} else {
        stopInfo.atStop = (stopInfo.eta <= stopInfo.timestamp) ? true : false;
    //}

    stopInfo.sequenceNumber = parseInt(GTFSrt.getCurrentStopSequenceForTrip(trip_id));

    if (isNaN(stopInfo.sequenceNumber)) {
        seqNumbers = parseInt(GTFS.getSequenceNumbersForStopForTrip(trip_id, stopInfo.stopId));
        
        stopInfo.sequenceNumber = (Array.isArray(seqNumbers)) ? parseInt(seqNumbers[0]) : null;
    }
...
function getLineStringBetweenStopsForTrip (_GTFS, gtfsTripKey, immediateStopId, subsequentStopId) {
    ...
    if ( ! Array.isArray(immediateStop_coords) ) {
        ...
        console.log('====================== Error ======================');
        console.log('tripKey:', gtfsTripKey, ', immedStopId:', immediateStopId, ', subsStopId:', subsequentStopId);
...
if (linestring.properties.line_distance_km < 0) {
    console.log('######### Negative line_distance_km ########');
    console.log('\tratioCovered =', ratioCovered) ;
}
