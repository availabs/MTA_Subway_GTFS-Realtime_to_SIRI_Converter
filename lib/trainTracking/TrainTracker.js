 ...
 * NOTE: Train tracker initial state is for testing/debugging.
 */
function newSnapshot (GTFS, GTFSrt, config, trainTrackerInitialState) {
    var gtfsrtTimestamp = GTFSrt.getTimestampForFeedMessage(),
        prevTimestamp ,
        snapshot;

    previousSnapshot = previousSnapshot || trainTrackerInitialState || null;
...
//debugger;
    ...
    //FIXME: THIS CACHE IS NEVER CLEANED!!!
    //this.tripKeyToGTFSVersion = (previousSnapshot && previousSnapshot.tripKeyToGTFSVersion) || {};
    this.tripKeyToGTFSVersion = {};
...
Snapshot.prototype = {
    ...
    getDistanceFromCall : function (gtfsTripKey, stop_id) {
        ...
        var _GTFS = this.tripKeyToGTFSVersion[gtfsTripKey] || null ,
...
module.exports = {
    newSnapshot : newSnapshot ,
        ...
        //if (!this.trainLocations) { return null; }

        //var _this = this,
            //tripKeys = Object.keys(_this.trainLocations) ;

        //return tripKeys.reduce(function (acc1, tripKey) {
            //var trackingDataForTrain = _this.trainLocations[tripKey] , 
                //keys = trackingDataForTrain ? Object.keys(trackingDataForTrain) : null;

            //if (!keys) { return acc1; }


            //acc1[tripKey] = keys.reduce(function (acc2, key) {
                //if (key === 'locationGeoJSON') {
                    //if (!trackingDataForTrain.locationGeoJSON) { return acc2; }

                    //var geoJSONKeys = Object.keys(trackingDataForTrain.locationGeoJSON);
                    
                    //geoJSONKeys.reduce(function (acc3, geoJSONKey) {
                        //console.log(geoJSONKey);
                        //if (geoJSONKey === 'properties') {
                            //acc3.properties = _.omit(trackingDataForTrain.locationGeoJSON.properties, '_GTFS') ;
                        //} else {
                            //acc3[geoJSONKey] = trackingDataForTrain.locationGeoJSON[geoJSONKey];
                        //}

                        //return acc3;
                    //}, acc2.locationGeoJSON = {});

                //} else {
                    //acc2[key] = trackingDataForTrain[key] ; 
                //}
